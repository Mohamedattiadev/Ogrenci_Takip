-- validate_dormitory_relationships() satir arama sorgularini (Group/Student/TeacherAssignment)
-- caginin RLS gorunurluguyle calistiriyordu (SECURITY DEFINER yoktu). Bugune kadar bu satirlari
-- INSERT/UPDATE edebilen tek roller (admin, superadmin) zaten kendi kurumundaki her Group/Student
-- satirini gorebildigi icin sorun cikmiyordu. Hoca artik GroupMembership'e yazabildigi icin
-- (bkz. teacher_move_* RLS policy'leri, dormitory-policies.sql, migration 20260916150000),
-- hocanin gormedigi (ogretmedigi) bir gruba/ogrenciye referans veren gecersiz bir deneme, once
-- RLS'in temiz "42501 insufficient_privilege" reddine ulasmadan, tetikleyicinin
-- "SELECT ... INTO STRICT" satiri sifir satir bulup genel bir "no rows" hatasiyla patlardi.
-- SECURITY DEFINER, tetikleyicinin kurallari her zaman gercek veriye gore (rolun gorebildigine
-- degil) degerlendirmesini saglar - butunluk kurali zaten budur, govde degismedi.
CREATE OR REPLACE FUNCTION validate_dormitory_relationships() RETURNS trigger
...
BEGIN
  ...
  ELSIF TG_TABLE_NAME = 'TeacherAssignment' THEN
    IF (TG_OP = 'INSERT' OR NEW."isActive") AND NOT EXISTS (SELECT 1 FROM "User" WHERE id = NEW."teacherId" AND role = 'TEACHER' AND "isActive" AND "deletedAt" IS NULL) THEN
